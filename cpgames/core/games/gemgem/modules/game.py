...
class gemSprite(pygame.sprite.Sprite):
    def __init__(self, image, gem_type, size, position, downlen, **kwargs):
        ...
        while True:
            ...
            for x in range(self.cfg.NUMGRID):
                ...
                for y in range(self.cfg.NUMGRID):
                    ...
                    gem = gemSprite(image=self.gem_imgs[gem_type], gem_type=gem_type, size=(self.cfg.GRIDSIZE, self.cfg.GRIDSIZE), position=[self.cfg.XMARGIN+x*self.cfg.GRIDSIZE, self.cfg.YMARGIN+y*self.cfg.GRIDSIZE-self.cfg.NUMGRID*self.cfg.GRIDSIZE], downlen=self.cfg.NUMGRID*self.cfg.GRIDSIZE)
        ...
        if res_match[0] == 1:
            ...
            while start > -2:
                for each in [res_match[1], res_match[1]+1, res_match[1]+2]:
                    ...
                    if start == res_match[2]:
                        ...
                        gem_type = random.choice(list(self.gem_imgs.keys()))
                        gem = gemSprite(image=self.gem_imgs[gem_type], gem_type=gem_type, size=(self.cfg.GRIDSIZE, self.cfg.GRIDSIZE), position=[self.cfg.XMARGIN+each*self.cfg.GRIDSIZE, self.cfg.YMARGIN-self.cfg.GRIDSIZE], downlen=self.cfg.GRIDSIZE)
                        self.gems_group.add(gem)
                        self.all_gems[each][start+1] = gem
                start -= 1
        elif res_match[0] == 2:
            ...
            while start > -4:
                if start == res_match[2]:
                    ...
                    gem_type = random.choice(list(self.gem_imgs.keys()))
                    gem = gemSprite(image=self.gem_imgs[gem_type], gem_type=gem_type, size=(self.cfg.GRIDSIZE, self.cfg.GRIDSIZE), position=[self.cfg.XMARGIN+res_match[1]*self.cfg.GRIDSIZE, self.cfg.YMARGIN+start*self.cfg.GRIDSIZE], downlen=self.cfg.GRIDSIZE*3)
                    self.gems_group.add(gem)
                    self.all_gems[res_match[1]][start+3] = gem
                start -= 1
    '''移除匹配的gem'''
