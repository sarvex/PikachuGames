...
class GamingInterface():
    ...
    def drawEnemies(self, screen):
        for enemy in self.enemies_group:
            if enemy.life_value <= 0:
                ...
                del enemy
                continue
            if res := enemy.move(self.element_size):
                if coord := self.find_next_path(enemy):
    ...
    def drawToolbar(self, screen):
        ...
        money_info = self.info_font.render(
            f'Money: {str(self.money)}', True, (255, 255, 255)
        )
        health_info = self.info_font.render(
    ...
    def showSelectedInfo(self, screen, button):
        if button.text in ['T1', 'T2', 'T3']:
            ...
            selected_info1 = self.info_font.render(
                f'Cost: {str(turret.price)}', True, (255, 255, 255)
            )
            selected_info2 = self.info_font.render(
                f'Damage: {str(turret.arrow.attack_power)}', True, (255, 255, 255)
            )
            selected_info3 = self.info_font.render(
                f'Affordable: {self.money >= turret.price}', True, (255, 255, 255)
            )
    ...
    def buildTurret(self, position):
        ...
        if (
            self.money - turret.price >= 0
            and self.current_map.get(turret.coord) in self.placeable.keys()
        ):
    ...
    def find_next_path(self, enemy):
        ...
        return next(
            (
                ...
                if (neighbour in self.path_list)
                and (neighbour not in enemy.reached_path)
            ),
            None,
        )
